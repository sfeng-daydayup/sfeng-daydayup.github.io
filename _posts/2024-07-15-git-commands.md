...
- 开源  
- 性能  
  这点有点存疑，当然也是codebase大到一定程度的时候遇到过需要增加DDR，定期清缓存之类的问题  
- 安全性和完整性  
...
&emsp;&emsp;前两个通过github上实践验证，所有的history都保留完好，并且commit message和commit id都与原先一样。:satisfied:第三个方案因为不是administor，没有实践，理论上应该可行。   
